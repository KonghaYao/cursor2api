...
- `composer-2.5`：Composer 标准档。AgentService 上这是**家族 id**，必须显式 `parameters.fast=false`，否则 Cursor 默认 Fast，Team Usage 会记成 `composer-2.5-fast`
- `composer-2.5-fast`：Composer Fast（`parameters.fast=true`）
- `grok-4.6` / `grok-4.6-fast`：同样去掉末尾 `-fast` 再交给上游，用 `parameters.fast` 区分；带 tools 时自动 Fast
...
- `usage`：OpenAI 为 `prompt_tokens` / `completion_tokens` / `prompt_tokens_details.cached_tokens`；Anthropic 为互斥的 uncached / cache read / cache write。来自 AgentService `turnEnded`，不是 Inference `usage` 帧
...
| 响应里的 usage | `usage.prompt_tokens` + `prompt_tokens_details.cached_tokens` | 从 AgentService `turnEnded`（`inputTokens` / `outputTokens` / `cacheReadTokens` / `cacheWriteTokens`）映射。`tool_calls` 那一枪 turn 尚未结束，usage 常为 0；最终文本那一枪带上整轮。上游若不带字段则仍为 0；Team Usage CSV 仍是计费真源 |
...
- **2026-09-09 — Inference 上游事故（L）**：Cursor 对 Dashboard `crsr_` 的 `aiserver.v1.InferenceService/Stream` 回 `ERROR_NOT_LOGGED_IN`。换票和模型列表仍可能成功，因此不能用 `/v1/models` 判断推理是否还能打。聊天已改到 `agent.v1.AgentService/Run` + 进程内 customTools（**无** `@cursor/sdk`、**无** agent 二进制、**无** Cloud sandbox VM）。本机 Deno 实机：OpenAI probe 7/7、Anthropic `system`、Grok Fast `PONG`。详见 `CLAUDE.md` **INC-2026-09-09**。
