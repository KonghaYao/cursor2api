...
| 流式 `stream: true` | 可用 | 立刻回 SSE；正文/思考在 AgentService 推出 `textDelta`/`thinkingDelta` 时转发。增量帧保持 OpenAI 形状（不在每帧重复 `conversation_id`）；上游空闲 ≥15s 才发 keepalive。Composer 常在想完后短时间打出全文。工具调用仍是一整块 |
