...
| roots 政策 | **独立 first root**：短目录 `Tools: Write, Edit, Bash.`；目录含 Write/Edit/StrReplace 时写明 **You have full read and write access**，立刻 Write/Edit。工具结果跟进枪的 `userMessageAction` 再复述一遍（政策在长 history 后看不见）。不要讲 MCP / custom-user-tools。不要写「Native Write is disabled」 |
